...
/// (time-aware greeting, one section label, one subtitle, one primary action, one stat) —
...
/// is disabled while baseline-metrics is the active focus — see `Docs/BaselineAssessment_Design.md`.
/// `SpatialRehabApp` still declares the `ImmersiveSpace` scene and owns `teaSession`, so
/// that work isn't deleted, just not entered from here for now. The primary action opens
/// `CaregiverDashboardView` — a real feature now (trend charts across sessions), not the dev
/// preview it used to be; the raw `BaselineResultsDebugView` dump moved to a toolbar button
/// inside that dashboard instead of being reachable directly from here.
